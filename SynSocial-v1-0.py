...
TODAY = datetime(2020, 1, 1, 00, 00, 00)  # Date of last Twitter message (Tweet),
...
excelRefDataFileName = "rate-info.xlsx"  # Reference data Excel Workbook file name - name
...
excelCodeContentFileName = "content.xlsx"  # The file that contains the messages contents
# for each condition code Twitter message
excelBaseContentFileName = "baseline.xlsx"  # The file that contains the messages contents
# for each baseline Twitter message
randomNounsFileName = "nounlist.txt"  # The file that contains words used to create the
# Twitter screen name
fileNamePhrases = "phrases.xlsx"  # The file that contains phrases added to the
...
def record_stats_day (input_day, input_file_name, num_generation, input_type):
    """
    ...
    patientShortFilename = input_file_name [0:9]
    rowDayInputCompare = input_day.strftime("%m/%d/%Y")
    ...
    lastRow = outputStatusWorksheet.max_row
    lastCol = outputStatusWorksheet.max_column
    for i in range (3, lastRow+1):
        testcell1 = outputStatusWorksheet.cell(row=i, column=1).value
        if (str(rowDayInputCompare) == str(testcell1)):
            #outputStatusWorksheet[row,col] = num_generation
            for j in range (2, lastCol+1):
                testcell2 = outputStatusWorksheet.cell (row = 2, column=j).value
                if (str(patientShortFilename) == str(testcell2)):
...
def obtain_mean_base_rate(input_age):
    ...
    try:
        code_reference_workbook = xlrd.open_workbook("{}\\{}".format(localRefDataFilePath,
                                                                     excelRefDataFileName))
        try:
            base_reference_workbooks = code_reference_workbook.sheet_by_name(excelRefBaseWks)

            col_a = base_reference_workbooks.col_values(0, 1)
            col_b = base_reference_workbooks.col_values(1, 1)
            reference_base_rate = {a: int(b) for a, b in zip(col_a, col_b)}

            if lookup_age < 18:
                base_mean_out = 0
                base_range_out = "under18"
            elif lookup_age < 20:
                base_mean_out = reference_base_rate["18to20"]
                base_range_out = "18to20"
            elif lookup_age < 25:
                base_mean_out = reference_base_rate["20to25"]
                base_range_out = "20to25"
            elif lookup_age < 30:
                base_mean_out = reference_base_rate["25to30"]
                base_range_out = "25to30"
            elif lookup_age < 40:
                base_mean_out = reference_base_rate["30to40"]
                base_range_out = "30to40"
            elif lookup_age < 50:
                base_mean_out = reference_base_rate["40to50"]
                base_range_out = "40to50"
            elif lookup_age < 60:
                base_mean_out = reference_base_rate["50to60"]
                base_range_out = "50to60"
            elif lookup_age < 70:
                base_mean_out = reference_base_rate["60to70"]
                base_range_out = "60to70"
            elif lookup_age < 85:
                base_mean_out = reference_base_rate["70to85"]
                base_range_out = "70to85"
            else:
                base_mean_out = reference_base_rate["85to99"]
                base_range_out = "85to99"

        except XLRDError:
            sys.stdout.write("No such Workbook in file: %s\n" % excelRefBaseWks)
    except IOError:
...
def obtain_mean_code_rate(input_condition_code):
    ...
    try:
        code_reference_workbook = xlrd.open_workbook(
            "{}\\{}".format(localRefDataFilePath, excelRefDataFileName)
        )

        try:
            code_reference_worksheet = code_reference_workbook.sheet_by_name(excelRefCodeWks)

            num_rows = code_reference_worksheet.nrows
            code_found = False
            for i in range(1, num_rows):
                row = code_reference_worksheet.row_values(i)
                rowtest = row[0]
                rowtested = int(rowtest)
                rowtestedstr = str(rowtested)
                if rowtestedstr == "":
                    print("blank row")
                if input_condition_code == "":
                    print("blank code")
                if rowtestedstr == input_condition_code:
                    mean_out = int(row[1])
                    alpha_out = float(row[2])
                    code_found = True
                    break
            if not code_found:
...
def codes_in_effect(input_list_codes, input_age):
    ...
    for i in range(0, num_codes):
        ...
        alpha_list.append(alpha)  # the impact (alpha) to the baseline message generation rate is the minimum alpha value assigned to all codes

    baseline_name, base_mean_raw = obtain_mean_base_rate(input_age)  # Call baseline rate function
    alpha_min = min(alpha_list)
    base_mean = base_mean_raw * alpha_min
    code_b_dict = {
        'code': baseline_name,
        'meanvalue': base_mean,
        'type': 'baseline'
...
def gen_screen_name(input_name):
    ...
    try:
        ...
        with open("{}\\{}".format(localRefDataFilePath, randomNounsFileName), 'r') as nounlist:
            for line in nounlist:
                num_nouns = num_nouns + 1
            nounlist.seek(0)
            seed = num_nouns
            random_sel = randint(0, num_nouns)
            noun = nounlist.readlines()[random_sel]
            tweet_screenname_output = "@" + input_name[0:6] + noun[0:7]
            return tweet_screenname_output
    except FileNotFoundError:
        print("Nounlist file does not exist")
        return ""
...
def output_tweet_json(
        ...
        input_screenname, input_user_id, input_location, input_file_name):
    """
    ...
    filename = input_file_name[
               :-5] + "TwitterData" + ".json"  # file name = original filename
    # with TwitterData appended on the end
    with open("{}\\{}".format(localOutputFilePath, filename),
              "a", encoding="utf-8") as outfile:
        outfile.write(json.dumps(fundemental_tweet, ensure_ascii=False))  # generated
        # tweet appended to the file
    return
...
def gen_twit_date_time(input_day):
    ...
    hour_seed = now.hour
    seed = hour_seed
    hour_value = randint(0, 23)

    # randomly generate minute
    minseed = now.minute
    seed = minseed
    min_value = randint(0, 59)

    # randomly generate second
    secseed = now.second
    seed = secseed
    sec_value = randint(0, 59)
...
def gen_twit_id_str(input_created_at):
    ...
    seed = microseed
    rand_ip2 = randint(1, 254)
    rand_ip3 = randint(1, 254)
    rand_ip4 = randint(1, 254)
...
def obtain_tweet_text(input_code, input_type):
    """
     ...
     Text is randomly selected from a row in an Excel file
     A random phrase is added to the start and end of each text 
     One file containing all the Tweets each Patient is Created
    """

    global excelBaseContentFileName
    global excelCodeContentFileName
    if input_type == "baseline":
        excel_content_filename = excelBaseContentFileName
    elif input_type == 'code':
        excel_content_filename = excelCodeContentFileName
    else:
        output_text_blank = " "
        return output_text_blank
    sheet = str(input_code)

    try:
        df_code_textlist = pd.read_excel("{}\\{}".format(localRefDataFilePath,
                                                     excel_content_filename), sheet_name=sheet) 
    except XLRDError:
        sys.stdout.write ("No such code in Worksheet: %s\n" %sheet)
        blank_output_text = " "  
        return blank_output_text
    #if the spreadsheet doesn't contain a sheet and text for the code, a blank message is returned

    num1 = len(df_code_textlist) - 1

    # randomly select the message
    now = datetime.now()
    microseed = now.microsecond
    seed = microseed
    sel_text = randint(0, num1)
    col_t = 0
    row_t = sel_text
    text_selected = df_code_textlist.iat[row_t, col_t]

    # randomly select start and end phrase
    # from the list in phrases.xlsx, start and end sheets
    ...
    sel_start_text = randint(0, num2)
    sel_end_text = randint(0, num3)
    col_p = 0
...
def read_FHIR_dates():
    ...
    if os.path.exists(localDataFilePath):
        ...
        for filename in os.listdir(localDataFilePath):
            ...
            with open("{}\\{}".format(localDataFilePath, filename), "r") as f:
                try:
                    data = json.load(f)
...
def obtain_tweet_conditions(data_dates_twitter):
    ...
    for patdata in data_dates_twitter:
        ...
        for filenamelist in os.listdir(localDataFilePath):
            ...
            with open("{}\\{}".format(localDataFilePath, filenamelist), "r") as f:
...
def generate_tweets(data_dates_twitter):
    """
     ...
     Text of Message = column [1] in content.xlsx of worksheet with tab that matches the code and 
      in column[1] in baseline.xlsx with tab matching age range
    ...
    for item in data_dates_twitter:
        ...
        #######
        #num_dates_tweet = 10  # Override for test #######
        # remove comment on previous line to limit the number of messages generated to 10
        #######
        ...
        for i in range(0, num_dates_tweet):
            ...
            for j in range(0, num_condition):
                ...
                abatement_datetime_formatted = datetime.strptime(item['conditions'][j]["abatementDateTime"],
                                                                "%Y-%m-%d").date()
            ...
            for code in effective_code_date:
                ...
                num_generation = poisson.rvs(mu=mean, size=1)  # The number generated is 
                # Poisson Distributed from the Mean
               
                record_stats_day(input_day, input_file_name, num_generation, input_type) 
                #Capture Statistic - Number of baseline and medical condition messages generated on that day for that user
                
                for _ in range(0, int(num_generation)):
                    output_tweet_json(input_day, input_code, input_type, input_name,
                                      input_screenname, input_user_id,
                                      input_location, input_file_name)  # call the function to
                    # generate and output the Tweet
